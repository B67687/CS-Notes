...
Given $n$ binary inputs, it activates one of up to $2^n$ outputs—typically in **[one-hot](../1-one-hot-vs-one-cold-encoding/)** form

{{< callout type="info" >}}

- **Inputs**: $n$ binary signals
- **Outputs**: $2^n$ one-hot lines, each a [ minterm](../minterms-and-maxterms/#minterm) of the input space
{{< /callout >}}
```mermaid
graph TB
  subgraph Inputs
