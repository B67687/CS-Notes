...
- **Status report after every round.** After each execution round (tool batch, subagent return, verifiable milestone), pause to report: what was done, what changed, what's next. Keep it 2-4 lines. This prevents context drift and gives the user a steering point. Slot it as a natural break after the verification step, before advancing.
